---
layout: default
title:  The Unacceptium Github pages
---

<script src="https://keepandroidopen.org/banner.js?size=minimal&animation=off"></script>


**i do not play msts. i do not play omsi. i have directly written a list of the games i play and can help in/make liveries in on the site you probably got redirected from. please ask someone else if you want liveries or help in microfos train simulator MSTS, omnibus simulator OMSI, or anything else what is not hmmsim, luanti or proton bus.** i have **never** played those games, **never** will because i have no computer and they are paid. **i cannot help or make liveries or mods in those games.**

### thank you for your understanding of the statement above, and be aware that your message will be deleted, you will be banned and blocked otherwise.

---

![favicon.ico](https://unacceptium.github.io/favicon.ico "favicon")**Welcome on my Github Pages site!**  
![youtube banner](https://yt3.googleusercontent.com/JUpGA_oICQH4ZLN7S5il_Q-C3tscnXSfDmf2-qkuJ6lPkAlReiZKG0Bm2KP2CR2_Wbga9dgbgoY=w1060-fcrop64=1,00005a57ffffa5a8-k-c0xffffffff-no-nd-rj "youtube-banner")  
  
I do photo edits, skins/liveries for games, simple addons, and gameplay content. this site is for collecting all of them.  
  
![alt text](https://cdn.rory.server.spacebar.chat/banners/1475727624237846074/2d48f599298b8fbb3d1995bebc051aaa.png?size=160&keep_aspect_ratio=false&animated=true "banner2")  
  
stuff look a bit weird by now.
- android is [locking down,](https://keepandroidopen.org/) and since i rely on developer builds, i am fked.
- for some reason no good pbsu mods are surfacing lately, and they instantly get skins i would lile to make.

idk what gonna happen. idk if updating this site has any use at all, i don't think anyone reads this at all. i have no inspiration lately, lost the metrotest map to top it off, and my workplace also fked things up so i don't even have energy to do pbs or minetest stuff at all.  
  
**this does not mean i don't accept requests for skins or mod edits. i just won't make any unless someone directly asks.**  
  
**Quick links**
[Navigation](https://unacceptium.github.io/#navigation)  
[About](https://unacceptium.github.io/#about)  
[Current project](unacceptium.github.io#current-projects)
[Contact](https://unacceptium.github.io/#contact)  
[Current hardvare](unacceptium.github.io#current-hardvare)  
  
### Navigation

if you want to request a proton bus skin, advtrains livery, feel free to make an issue on this site's repo, comment on my yt videos or send a direct message on any of the other sites found here.
**on-site**
  
[Proton Bus Skins, Edits](pbsu.md)  
Repaints for buses, configurations fixes like collision box sizes, and youtube videos showcasing all of them.
  
[Luanti content](luanti.md)  
AdvTrains liveries, simple mods, and the Metrotest map.

[Metrotest Map Project](https://unacceptium.github.io/luanti.html#metrotest-map-project)  
A Luanti AdvTrains metro system where the player drives metro trains. the world is more than 3 years old, still getting updates, and the line 1 takes almost a hour to go through.

**off-site**
  
[Youtube channel](https://youtube.com/@unacceptium_core)  
Mainly showcase and gameplay videos.
  
[Tömegközlekedés másKÉPp](https://forum.index.hu/Article/showArticle?t=9051694&la=170688502)  
a very old forum where we uploaded edits of buses, ex. triarticulated buses or levitating ones, you know... (very inactive and kind of toxic, not recommended. also it is operated by a politic-weighted news site, imagine the moderation and topics) 

[Ununennium Járműgyártó és -felújító CG](about:blank)  
**W.I.P.**
Inspired by fictional bus brands like [Verzon](https://www.facebook.com/VerzonGroupSA), [Cee](https://explozaogamer.com.br/2023/06/19/mod-cee-neweco-mbb-eo-500u-fase-2), [Busheme](https://explozaogamer.com.br/2023/03/23/mod-busheme-velsus-800-6x4-fase-2) and [Techno](https://explozaogamer.com.br/2023/11/03/mod-techno-raffica-mp450s-articulado-fase2), i started making a fictional bus brand. while it is far from presentable state by now, i can say that my buses gonna feel more like Volvo 8700a, latest gen Citywide-le or MAN A40, keeping the simple rectangle shape, intercity low entry articulated build with iconic designs. but that doesn't exclude tramtrains, trucks or bus rapid transit...  
  
**others of my interests**
  
[Időkép](https://www.idokep.hu/keptar/user/unacceptium)  
Site where we upload photos of nature and weather. Main feature is weather.  

[FindAFox](https://findafox.net/user/Unacceptium)  
site collecting photography of foxes. creator intended the site to collect the most beautiful, open licensed photography but be aware that most of the new users are unable to follow this, neither can they tag correctly resulting in blacklists being useless. i sadly seen this happen on other sites…  
  
### Contact

- Discord:`Unacceptium_core`  (online almost daily)
- Stoat (formely Revolt):`Unacceptium_Core#0426`  (online almost daily)
- Spacebar/Fermo: `Unacceptium#1463`

### Current projects

| Evertech Sandbox | Luanti
|----|---|
| Guided LightTransit (stopped) | Metrotest (still trying to recover project) |
| Buses (in progress, soon YT videos) | - |
| curb-guided bus (mono, center rail)(presentable, soon YT video) | - |

  
### About
#### where you could see the name
- i was a player on luanti servers UGX, creative trains, and was a mod on EOM dev's season 1
- i had an instagram account with about 112 followers, i got banned for _racism XD_
- i had a reddit account, posted in luanti mainly, mostly about metrotest
- i had a precedessor to metrotest in minecraft, it used minecarts and i used a different name back then, also appeared in multiple minecraft servers
- i was an experienced player in _amogsus_, sometimes i was so good that i got banned for hacking despite not doing that. lately i switched to playing hide and seek mode only, then left because lobbies became unfair. my name was garlic and i got a meme made about me for talking with ghosts (i had a friend dead ingame who told me what ghosts were talkin about)
- translated Luanti:Piranesi Restoration Project EN-HU
- partly translated Luanti EN-HU
- translated Fulguris Webbrowser EN-HU
- waiting for luanti website to finally merge my hungarian translation for it…
#### my interests
- I'm a fan of artiulated low entry intercity spurbuses and low entry tramtrains. i favorite diesel, inline hybrid, wired electric with diesel generator and dualmode powertrains.
- My profession is welder, i learned tungsten, stick, mag and flame welding.
- i got TPAW and TPEB robot programming and operating course. the factory seemed to be really wanting me to be a robot operator but they got me off the robot for no reason.
- i like those old experimental and feature phones, like slide-out keyboards and rotate-out cameras, my current phone has some quirks and gimmics too, so i kind of kept this interest to today
- my music taste consists of:  
  - melodic and raw extratone-ultratone, splittercore
  - melodic dubstep, hardstyle, rawstyle
  - metalstep, filthstep, death and trash metal
- i am from 🇭🇺's capital, especially one of those locations where Lion's City A21 and AG/NewAG300 buses turn around.
- i like foxes, mainly cross, original red, silver and darwin ones. i got this interest a while ago, and still not completely sure why i like them (because i kind of dislike animals in overall), but since they are not humans, they have no human problems and probably thats the main reson outside their look and behaviour, what i also like.  

### Current hardvare
this lists the devices i use to make content. i moved this section down as most people probaly don't care.

| Name   |  Image  | Function |
|----|----| -- |
| Siemens [Gigaset GX4 Pro](https://www.gigaset.com/hq_en/gigaset-gx4-pro/) | ![alt text](https://www.gigaset.com/media/catalog/product/cache/36/image/1800x/040ec09b1e35df139433887a97daa66f/s/m/smartphone-display-gx4-pro_black_2024_low-res_2.jpg "official photo")   | Uploading, photo editing, resource gathering (a legend!) |
| [Honor Pad X8A](https://www.honor.com/hu/tablets/honor-pad-x8a/) | ![alt text](https://www-file.honor.com/content/dam/honor/hu/products/tablets/honor-pad-x8a/assets/imgs/section-1/sec1-bgd-mob.jpg "photo") | Gaming, screenrecording, video editing (acceptable) |
| PAST | DEVICES | IN ORDER |
| [Ulefone Armor 15](https://www.ulefone.com/products/armor-15) | ![alt text](https://cdn.shopifycdn.net/s/files/1/0830/0573/5193/files/m-product-armor-15-1.jpg?v=1705039484 "photo") | Everything (was disguistingly terrible, even if the design is legendary) |
| [Honor X7A](https://www-file.honor.com/content/dam/honor/hu/products/smartphone/honor-x7a) | ![alt text](https://www-file.honor.com/content/dam/honor/hu/products/smartphone/honor-x7a/imgs/sec-1/sec-1-1-mob1.jpg "") | Everything (weren't too good) |
| Siemens [Gigaset GS190](https://web.archive.org/web/20190916004736/https://www.gigaset.com/de_de/gigaset-gs190/) | ![alt text](https://media.s-bol.com/mO0L6B26Z3VR/550x550.jpg "") | Everything (was a legend!) |


---

[GH Repo](https://github.com/Unacceptium/Unacceptium.github.io) _last update:2026.07.04_